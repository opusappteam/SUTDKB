   ...
   **/
   function showSummary($ticket) {
     ...
     /*  $query = "SELECT `glpi_ticketfollowups`.*, `glpi_users`.`picture`
	   ...
	   // AKK 25 MARCH 2015 4:24 PM ORDER by Older to NEW
      ...
      if ($DB->numrows($result) == 0) {
         ...
         while ($data = $DB->fetch_assoc($result)) {
            ...
            echo "<div class='boxnoteleft'>";
              ...
              //AKK mod for Followup image disable on 06april 2015

		 /*  echo "<img class='user_picture_verysmall' alt=\"".__s('Picture')."\" src='".
                User::getThumbnailURLForPicture($data['picture'])."'>"; */
            echo "</div>"; // boxnoteleft

           //AKK mod for Followup color change
		  echo "<div  align='left' class='boxnotecontent' style='background-color: #D8D8D8' ";
            echo ">";
			...
			//mod by akk for space in followups on 06 april 2015
			else{
			echo("<br>");
			}
