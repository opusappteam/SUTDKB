   ...
   **/
   static function formatNumber($number, $edit=false, $forcedecimal=-1) {
      ...
      switch ($_SESSION['glpinumber_format']) {
         ...
         default: // English
            // Update AKK 07 April 2015 for Remove Comma in Ticket
			//return number_format($number, $decimal, '.', ',');
			return number_format($number, $decimal, '.', '');
      }
   }
...
//          echo 'menu load';
   ...
   **/
   static function showToolTip($content, $options=array()) {
      ...
      //AKK update: Disable tooltip in new template on April 07 2015
	  
	  
	 // $out .= "<div id='".$param['contentid']."' class='invisible'>$content</div>";
	      $out .= "<div id='".$param['contentid']."' style='display: none;'>$content</div>";
      if (!empty($param['popup'])) {
